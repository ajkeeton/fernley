...
public:
  ...
  void init(int i, int en, int step, int dir, int lsl, int lsh) {
    ...
    set_target(-INT_MAX); // set target to low val so we find the lower limit ... 0xffffffff didn't work for some reason 
  }
  ...
  void set_forward(bool f) {
    if(forward != f) {
      tpause = 3000; // 30 ms before reversing
      cur_num_steps = 0;
    }
...
void setup() {
  ...
  state[1].init(1, ENABLE_2, STEP_2, DIR_2, LIMIT_SWITCH_LOW_2, LIMIT_SWITCH_HIGH_2);
